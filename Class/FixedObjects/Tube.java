...
import Class.Sprite.FlowerThing;
import Class.Sprite.Mario;
import Class.Sprite.Sprite;

public class Tube extends StaticObject
{
	private boolean flowerThing;
	private FlowerThing flower;
	public Image image;
	private Boolean hasCave;
	
	public Tube(int x, int y, boolean flower, int width, int height, String img) throws IOException{
		super(x, y, width, height);
		image = ImageIO.read(new File(img));
	}
	
	public void portalToCave( Mario mario ){
	...
	public void collisionRight(Sprite sprite){
		...
		sprite.horizontalVelocity = 0;
		if(sprite.horizontalPosition <= this.horizontalPosition + this.width)
